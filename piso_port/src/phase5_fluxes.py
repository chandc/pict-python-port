...
def pressure_face_fluxes(p, J, metrics, h, coef=None, include_orth=True,
    ...
    rhie_chow=True returns the Rhie-Chow dissipation instead: the COMPACT face difference minus
    the face-interpolated WIDE cell gradient (`deriv`, the same stencil the momentum predictor
    uses). The two agree to O(h^2) for smooth p, so the difference is O(h^3 d4p/dx4) and leaves
    second-order accuracy intact; for a node-to-node alternating field the wide term vanishes
    while the compact term is maximal. See reference/pressure_checkerboard.md.
    """
    per = as_periodic(periodic)
